...
const vector<Command> COMMANDS = {
    ...
    {"^", 2, false, exponential},
    {"POWER", 2, false, exponential},
    {"EXP", 2, false, exponential},
    {"STOP", 0, true, nullptr},
    {"QUIT", 0, true, nullptr},
    {"EXIT", 0, true, nullptr},
    {"Q", 0, true, nullptr},
...
int main() {
    ...
    while ( true ) {
        ...
        for (unsigned long i=0; i < command_to_be_executed.length(); ++i) {
            command_to_be_executed.at(i) = toupper(command_to_be_executed.at(i));
        }

        for(auto& i:COMMANDS) {
            if (command_to_be_executed == i.str) {
                was_found = true;
                if (pieces.size() == 3 or pieces.size() == 1) {
                    if (i.is_exit) {
                        cout << GREETING_AT_END << endl;
                        return EXIT_SUCCESS;
                    ...
                    if (isdigit(pieces.at(1).at(pieces.at(1).size()-1)) and isdigit(pieces.at(2).at(pieces.at(2).size()-1))) {
                        cout << i.action(stod(pieces.at(1)), stod(pieces.at(2))) << endl;
                    } else {
