...
bool update_data_file(const std::string& file_name,
                      std::map< std::string, Student* > user_ids) {

    std::ofstream file_object(file_name);

    for (auto i:user_ids) {
        file_object << i.second->student_number << ";";
        file_object << i.second->user_id << ";";
        file_object << i.second->name << ";";
        file_object << i.second->phone_number << ";";
        file_object << i.second->email << ";" ;
        file_object << i.second->skype << std::endl;
    }
            ...
            if(parts.size() != 2){
                ...
                if (student_numbers.find(parts[1]) == student_numbers.end()) {
                    ...
                    if (is_valid_phone_number(number)) {
                        ...
                        std::cout << std::endl;
                        update_data_file(file_name, user_ids);
                    }
                }
            }
